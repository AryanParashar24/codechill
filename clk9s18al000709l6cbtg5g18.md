...
To create a GO test, create a file with the name say `math_test.go`. The GO compiler knows to ignore code in any files that end with `_test.go`, so the code defined in this file is only sued by the go test.
